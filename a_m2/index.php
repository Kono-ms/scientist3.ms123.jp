...
function DispData($mode,$sort,$word,$key,$page,$lid,$token)
{
	...
	if ($mode!="list"){
		...
		//決済者承がいるかどうかで「決済者承認」プルダウンの表示わけ
		$StrSQL="SELECT ID,MID FROM DAT_M3 where MID='".$FieldValue[122]."' ";
		...
		//echo "-->";
		if($kessai_num>=1){
			$str=DispParam($str,"KESSAISYA");
			$str=DispParamNone($str,"NO_KESSAISYA");
		}else{
			$str=DispParamNone($str,"KESSAISYA");
			$str=DispParam($str,"NO_KESSAISYA");
		}
