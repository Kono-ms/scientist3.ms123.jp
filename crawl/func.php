...
function get_all_url($url, $url_list){
	...
	foreach($html->find("a") as $obj){
		...
		if( !in_array( $link, array_column( $url_list, 0 ) ) ){
...
function get_info($url){
	...
	$info=array();
	$info["url"]=$url;
	$info["title"]=$html->find("title",0)->plaintext ?? "";
	$info["description"]=$html->find("meta[name=description]",0)->content ?? "";
	$info["keywords"]=$html->find("meta[name=keywords]",0)->content ?? "";
