...
//戻値 
//=========================================================================================================
function ListSql($sort,$word,$kbn,$date1,$date2,$price1,$price2,$currency,$customer_name)
{
	//extract($GLOBALS);
	eval(globals());
  ...
  if($kbn=="1"){
		$str=$str." AND DAT_FILESTATUS.STATUS='見積り送付' ";
	} else{
		$str=$str." AND DAT_FILESTATUS.STATUS='発注依頼' ";
	}
  ...
  if($date1!=""){
		$str=$str."  AND DATE_FORMAT(STR_TO_DATE(DAT_FILESTATUS.NEWDATE, '%Y/%m/%d %H:%i:%s'), '%Y%m%d%H%i%s') ";
		$str=$str."  >= DATE_FORMAT(STR_TO_DATE('".$date1."', '%Y-%m-%dT%H:%i:%s'), '%Y%m%d%H%i%s') ";
	}
	if($date2!=""){
		$str=$str."  AND DATE_FORMAT(STR_TO_DATE(DAT_FILESTATUS.NEWDATE, '%Y/%m/%d %H:%i:%s'), '%Y%m%d%H%i%s') ";
		$str=$str."  <= DATE_FORMAT(STR_TO_DATE('".$date2."', '%Y-%m-%dT%H:%i:%s'), '%Y%m%d%H%i%s') ";
	}
	if($price1!=""){
		// $str=$str."  AND CAST(MITSUMORISYO_ALL_CHARGE as signed) >= CAST('".$price1."' as signed) ";

		//合計金額(小計1+消費税1+小計2+消費税2)
		//小計1
		$str=$str."  AND CAST(";
		$str=$str."  ifnull(";
		$str=$str."  cast(FILESTATUS_DETAIL.M2_DETAIL_HANDLING_FEE as DECIMAL(20,3))";
		$str=$str."  + cast(FILESTATUS_DETAIL.M2_DETAIL_PRICE as DECIMAL(20,3))  ";
		$str=$str."  + cast(DAT_FILESTATUS.M2_SPECIAL_DISCOUNT as DECIMAL(20,3))";
		$str=$str."   ,0)";
		//消費税1
		$str=$str." + ";
		$str=$str."   ifnull(";
		$str=$str."  ((cast(CASE WHEN M1_DVAL04 = 'M1_DVAL04:Japan' THEN 10 ELSE 0 END as DECIMAL(20,3))";
		$str=$str."  * (cast(FILESTATUS_DETAIL.M2_DETAIL_HANDLING_FEE as DECIMAL(20,3))";
		$str=$str."  + cast(FILESTATUS_DETAIL.M2_DETAIL_PRICE as DECIMAL(20,3))  ";
		$str=$str."  + cast(DAT_FILESTATUS.M2_SPECIAL_DISCOUNT as DECIMAL(20,3)))) / 100) ";
		$str=$str."   ,0)";
		//小計2
		$str=$str." + ";
		$str=$str."   ifnull(";
		$str=$str."   (cast(M2_ETC02 as DECIMAL(20,3))";
		$str=$str."  * (cast(FILESTATUS_DETAIL.M2_DETAIL_HANDLING_FEE as DECIMAL(20,3))";
		$str=$str."  + cast(FILESTATUS_DETAIL.M2_DETAIL_PRICE as DECIMAL(20,3))  ";
		$str=$str."  + cast(DAT_FILESTATUS.M2_SPECIAL_DISCOUNT as DECIMAL(20,3))) / 100) ";
		$str=$str."  + cast(M2_IMPORT_FEE as DECIMAL(20,3)) ";
		$str=$str."  + cast(MITSUMORISYO_EXPORT_FEE as DECIMAL(20,3)) ";
		$str=$str."  - cast(M2_MANAGE_DISCOUNT as DECIMAL(20,3)) ";
		$str=$str."   ,0)";
		//消費税2
		$str=$str." + ";
		$str=$str."   ifnull(";
		...
		$str=$str."  + cast(MITSUMORISYO_EXPORT_FEE as DECIMAL(20,3)) ";
		$str=$str."  - cast(M2_MANAGE_DISCOUNT as DECIMAL(20,3)) ) ";
		$str=$str."  * cast(DAT_FILESTATUS.M2_TAX_RATE2 as DECIMAL(20,3)) / 100) ";
		$str=$str."   ,0)";
		$str=$str."   as signed) ";
		$str=$str."  >= CAST('".$price1."' as signed) ";

	}

	if($currency!=""){
		$str=$str."  AND M2_CURRENCY = '".$currency."'";
	}
  
	if($price2!=""){
		// $str=$str."  AND CAST(MITSUMORISYO_ALL_CHARGE as signed) <= CAST('".$price2."' as signed) ";
		//合計金額(小計1+消費税1+小計2+消費税2)
		//小計1
		$str=$str."  AND CAST(";
		$str=$str."  ifnull(";
...
if($customer_name!=""){
		...
		$str=$str."  AND ( ";
		$str=$str."           M1_DVAL01 LIKE '%".$customer_name."%' ";
		$str=$str."        OR M2_DVAL03 LIKE '%".$customer_name."%' ";
		$str=$str."        OR M2_DVAL01 LIKE '%".$customer_name."%' ";
		$str=$str."        OR M2_DVAL16 LIKE '%".$customer_name."%' ";
		$str=$str."        OR M2_DVAL17 LIKE '%".$customer_name."%' ";
		$str=$str."        OR M2_DVAL18 LIKE '%".$customer_name."%' ";
		$str=$str."        OR M2_DVAL19 LIKE '%".$customer_name."%' ";
		$str=$str."  ) ";
	}
