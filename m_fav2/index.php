...
function Main()
{
	...
	if($_SESSION['MID']==""){
		$url="https://mabeauty.msc-dev.com/login2/";
		header("Location: {$url}");
		exit;
	}
...
function DispData($mode,$sort,$word,$key,$page,$lid,$token)
{
	...
	$filename="../common/template/listm1.html";
	$fp=$DOCUMENT_ROOT.$filename;
	$strM=@file_get_contents($fp);

	// SQLインジェクション対策
	$StrSQL="SELECT DAT_M1.* FROM DAT_M1 inner join DAT_IINE on DAT_IINE.MID=DAT_M1.MID and DAT_IINE.MIDT='".$_SESSION['MID']."' ";
	// 2021.03.15 yamamoto ENABLE:公開中のみ表示
	$StrSQL.=" and DAT_M1.ENABLE = 'ENABLE:公開中'";
	//2020/12/28 gaosan ADD START
	$StrSQL .= " and NOT EXISTS (SELECT * FROM DAT_BL WHERE DAT_BL.MID1 = '" . $_SESSION['MID'] . "' and DAT_BL.MID2 = DAT_M1.MID) ";
