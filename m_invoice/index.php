...
function Main()
{
		...
		case "save":
			...
			if ($_SESSION['token']==$token) {
				...
				if ($msg==""){
					...
					//スタータス 納品済・発注済 → 請求提示中
					if ($FieldValue[14]=="STATUS:納品済" || $FieldValue[14]=="STATUS:発注済" || $FieldValue[14]=="") {
						$FieldValue[14]="STATUS:請求提示中";
						$FieldValue[10]=date("Y/m/d H:i:s");
 
						$StrSQL="UPDATE DAT_ESTIMATES SET ";
						//$StrSQL=$StrSQL." CDATE ='".date("Y/m/d H:i:s")."',";
						$StrSQL=$StrSQL." STATUS ='STATUS:請求提示中'";
						$StrSQL=$StrSQL." WHERE EID ='".$FieldValue[2]."'";
					...
					$id="";
					$StrSQL=" SELECT ID FROM DAT_INVOICE";
					$StrSQL.=" WHERE IID = '".$iid."'";
					$StrSQL.=" AND EID = '".$eid."'";
					...
					$comment= '[請求書]が送信されました。' . "\n";
					$comment.="<a target=\"_blank\" href=\"/m_invoice/?mode=disp&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&eid=".$eid."&iid=".$iid."&key=".$id."\">Invoice</a>"; //請求書
	...
	DispData($mode,$sort,$word,$key,$page,$lid,$token,$mid1,$mid2,$eid,$iid,$itemdata,$quantity,$price,$etc);
...
//戻値 なし
//=========================================================================================================
function DispData($mode,$sort,$word,$key,$page,$lid,$token,$mid1,$mid2,$eid,$iid,$itemdata,$quantity,$price,$etc)
{
	...
	if ($mode!="list"){
        ...
        if ($FieldValue[14]=="STATUS:見積依頼中") {
			...
			$str=str_replace("[BACKURL]","https://scientist3.ms123.jp/m_chat1_list/?mode=list&word=".$word."&mid1=".$FieldValue[3]."&mid2=".$FieldValue[4]."",$str);
        } else if ($FieldValue[14]=="STATUS:発注済") {
			$str=str_replace("[SCREEN_TITLE]","Purchase Order",$str);
			$str=str_replace("[SCREEN_TITLE_DETAIL]","",$str);
        } else if ($FieldValue[14]=="STATUS:納品済") {
			$str=str_replace("[SCREEN_TITLE]","Invoice",$str);
			$str=str_replace("[SCREEN_TITLE_DETAIL]","Invoiceを作成してください。",$str);
        } else if ($FieldValue[14]=="STATUS:請求提示中") {
			$str=str_replace("[SCREEN_TITLE]","Invoice",$str);
			$str=str_replace("[SCREEN_TITLE_DETAIL]","",$str);
			$str=str_replace("[END_MSG]","Invoiceを作成しました。",$str);
			$str=str_replace("[BACKURL]","https://scientist3.ms123.jp/m_chat1_list/?mode=list&word=".$word."&mid1=".$FieldValue[3]."&mid2=".$FieldValue[4]."",$str);
        } else {
			$str=str_replace("[SCREEN_TITLE]","Invoice",$str);
			$str=str_replace("[SCREEN_TITLE_DETAIL]","",$str);
			$str=str_replace("[END_MSG]","",$str);
		}
        ...
        if ($mode=="new" || $mode=="edit") {
            ...
            if (is_array($itemdata)) {
				...
				if($mode=="new"){
					...
					$StrSQL=" SELECT * FROM DAT_INVOICE_DETAIL ";
					$StrSQL.=" WHERE IID ='".$iid."'";
				}
		...
		$StrSQL="SELECT * FROM DAT_M1 where MID='".$FieldValue[3]."'";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item=mysqli_fetch_assoc($rs);
		$str=DispM1($item, $str);
		$unit = $item["M1_ETC03"]; //通貨単位

		$StrSQL="SELECT * FROM DAT_M2 where MID='".$FieldValue[4]."'";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item=mysqli_fetch_assoc($rs);
		$str=DispM2($item, $str);
...
function LoadData2($key)
{
	...
	// SQLインジェクション対策
	// HTMLエスケープ処理（SQL読み込み）
	$StrSQL="SELECT * FROM DAT_ESTIMATES WHERE ID='".mysqli_real_escape_string(ConnDB(),$key)."';";
	$rs=mysqli_query(ConnDB(),$StrSQL);
...

{
	...
	if($item=="") {
		...
		$iid = "I".date("YmdHis").$msStr; //自動発番（I+YMDHIS+ミリ秒）
		$FieldValue[1] = $iid;
	...
	//DELETE
	$StrSQL="DELETE FROM DAT_INVOICE_DETAIL WHERE IID='".$FieldValue[1]."';";
	if (!(mysqli_query(ConnDB(),$StrSQL))) {
		die;
	}
	...
	if(is_array($itemdata)){
		for ($i=0; $i<count($itemdata); $i=$i+1) {
			...
			if($itemdata[$i]!="" || $quantity[$i]!="" || $etc[$i]!=""){
				...
				$StrSQL="INSERT INTO DAT_INVOICE_DETAIL (";
				$StrSQL.="IID,";
				$StrSQL.="ITEM,";
				$StrSQL.="QUANTITY,";
				$StrSQL.="UNIT,";
