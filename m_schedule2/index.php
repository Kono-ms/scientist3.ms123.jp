...
function DispData($mode,$sort,$word,$key,$page,$lid,$token,$sel1,$sel2,$word2,$mid1)
{
	...
	print $str;
		...
		$filename="../common/template/list_schedule2.html";
		$fp=$DOCUMENT_ROOT.$filename;
		...
		*/
		...
		//	WHERE
		//		DAT_FILESTATUS.MID2 = '".$_SESSION['MID']."' 
		...
		$StrSQL = "
			...
			WHERE
				...
				AND DAT_FILESTATUS.DIV_ID NOT LIKE '%Part0'
				AND DAT_FILESTATUS.MID1 != '".M1_SYSTEM_MID."'
			GROUP BY
				DAT_FILESTATUS.SHODAN_ID,
				DAT_FILESTATUS.ID,
				DAT_FILESTATUS.MID1
...
//echo '<!--main:'.$StrSQL.'-->';
		...
		if($item=="") {
			...
			while ($item = mysqli_fetch_assoc($rs)) {
				...
				//echo "<!--id:".$shodan_item["ID"].", status:".$shodan_item['STATUS']."-->";
		...
		//以下は仕様変更前のバージョンv1とする
		  ...
		  SELECT
			  ...
			  //group_concat(DAT_FILESTATUS.ID) as FILESTATUS_ID,
				//
			  min(DAT_FILESTATUS.ID) as FILESTATUS_ID,
...
//echo('<!--件数:'.$item.'-->');
		if($item=="") {
			...
			while ($item = mysqli_fetch_assoc($rs)) {
				...
				// TODO
				//$str=str_replace("[SHODAN_ID]",$item['SHODAN_ID'],$str);
				$str=str_replace("[SHODAN_ID]",$item['FILESTATUS_ID'],$str);
...
function MakeUrl($sort,$word, $sel1, $sel2,$page)
{
	...
	return "/m_schedule2/?mode=list&sort=".urlencode($sort)."&word=".urlencode($word)."&page=".urlencode($page)."&sel1=".urlencode($sel1)."&sel2=".urlencode($sel2);
