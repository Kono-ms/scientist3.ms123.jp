...
function Main()
{
	...
	if($mode=="disp"){
		...
		header("Location: ".BASE_URL . "/m_info2/".sprintf("%04d", $key)."/");
		exit();
	}
...
function DispData($mode,$sort,$word,$key,$page,$lid)
{
	...
	if ($mode!="list"){
		...
		if($FieldValue[4]!=""){
			...
			$str=str_replace("[PAN]", "<li class=\"breadcrumb__item\"><a href=\"/m_info2/".$item['URL']."/\">".str_replace("CCATE:", "", $FieldValue[4])."</a>&nbsp;&gt;&nbsp;</li>", $str);
			$str=str_replace("[CATE]", "<li class=\"breadcrumb__item\"><a href=\"/m_info2/".$item['URL']."/\">".str_replace("CCATE:", "", $FieldValue[4])."</a></li>", $str);
		} else {
		...
		if($item=="") {
			...
			for ($i=$s; $i<=$e; $i=$i+1) {
				if ($i==intval($page)) {
					...
					$str=$str."<li><a href=\"/m_info2/?mode=list&lid=".$lid."&sort=".$sort."&word=".$word."&page=".$i."\">".$i."</a></li>";
				} 
			}
	...
	if($word==""){
		...
		$item=mysqli_num_rows($rs);
		while ($item = mysqli_fetch_assoc($rs)) {
			$tmp.="<li class=\"breadcrumb__item\"><a href=\"/m_info2/".$item['URL']."/\">".$item['CNAME']."</a></li>";
//				$tmp.="<li style=\"padding:10px 0px;border-bottom:0px;\"><div class=\"info_lst\"><p class=\"info_lst_fl\"><a href=\"/m_info2/".$item['URL']."/\"><img src=\"".$item['CPIC']."\" alt=\"".$item['CNAME']."\" width=\"160\" /></a></p><p class=\"info_lst_fr\"><a href=\"/m_info2/".$item['URL']."/\">".$item['CCOMMENT']."</a></p></div></li>";
		} 
...
//		$StrSQL="SELECT * FROM DAT_CCATE where CNAME='".$word."' order by ID desc";
		...
		$item=mysqli_num_rows($rs);
		while ($item = mysqli_fetch_assoc($rs)) {
			$tmp.="<li class=\"breadcrumb__item\"><a href=\"/m_info2/".$item['URL']."/\" class=\"catebutton\">".$item['CNAME']."</a></li>";
//<li style=\"padding:10px 0px;border-bottom:0px;\"><div class=\"info_lst\"><p class=\"info_lst_fl\"><a href=\"/m_info2/".$item['URL']."/\"><img src=\"".$item['CPIC']."\" alt=\"".$item['CNAME']."\" width=\"160\" /></a></p><p class=\"info_lst_fr\"><a href=\"/m_info2/".$item['URL']."/\">".$item['CCOMMENT']."</a></p></div></li>";
		} 
		$str=str_replace("[CLIST]",$tmp,$str);
		...
		$str=str_replace("[PAN]", "<li class=\"breadcrumb__item\" itemprop=\"itemListElement\" itemscope itemtype=\"http://schema.org/Listitem\"><a href=\"/m_info2/\" itemprop=\"item\"><span itemprop=\"name\">記事コンテンツ</span></a>&nbsp;&gt;&nbsp;</li><li>".$word."</li>", $str);
	...
	$tmp="";
	$StrSQL="SELECT * FROM DAT_INFO where ETC04 like '%ETC04:Researchers向け%' AND CCATE like '".mb_substr($cate,0,10,"UTF-8")."%' and ETC12='ETC12:公開中' order by DATE desc, ID desc LIMIT 0, 8;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
		$tmp.="<a href=\"/m_info2/[INO]/\" class=\"similar_column\"><div><div class=\"similar_column_img\"> <img src=\"[PIC]\" alt=\"\"> </div><div class=\"similar_column_TITLE\"><p>[D-TITLE]</p><p>[COMMENT]</p></div><div class=\"similar_column_detail\"><p>[COMMENT]</p></div></div></a> ";
	...
	$cnt=1;
	$tmp="";
	$StrSQL="SELECT * FROM DAT_INFO where ETC04 like '%ETC04:Researchers向け%' AND CCATE<>'CCATE:特集' and ETC12='ETC12:公開中' order by ETC12, DATE desc, ID desc LIMIT 0, 10;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
		$tmp.="<a href=\"/m_info2/[INO]/\" class=\"ranking_column\"><div><div class=\"ranking_column_img\"> <img src=\"[PIC]\" alt=\"\"> <img src=\"/common/images/side_rank_[CNT].png\" alt=\"\"> </div><div class=\"ranking_column_TITLE\"> <img src=\"/common/images/side_rank_[CNT].png\" alt=\"\"><p>[D-TITLE]</p><p>[COMMENT]</p></div></div></a> ";
		$tmp=str_replace("[D-DATE]",$item['DATE'],$tmp);
		$tmp=str_replace("[D-CCATE]",str_replace("CCATE:", "", $item['CCATE']),$tmp);
		$tmp=str_replace("[PIC]",$item['PIC'],$tmp);
		$tmp=str_replace("[D-TITLE]",$item['TITLE'],$tmp);
	...
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
		$tmp.="<li><a href=\"/m_info2/[INO]/\" class=\"ranking_column\"><div><div class=\"ranking_column_img\"> <img src=\"[PIC]\" alt=\"\"> <img src=\"/common/images/side_rank_[CNT].png\" alt=\"\"> </div><div class=\"ranking_column_TITLE\"> <img src=\"/common/images/side_rank_[CNT].png\" alt=\"\"><p>[D-TITLE]</p><p>[COMMENT]</p></div></div></a></li> ";
		$tmp=str_replace("[D-DATE]",$item['DATE'],$tmp);
		$tmp=str_replace("[D-CCATE]",str_replace("CCATE:", "", $item['CCATE']),$tmp);
		$tmp=str_replace("[PIC]",$item['PIC'],$tmp);
		$tmp=str_replace("[D-TITLE]",$item['TITLE'],$tmp);
...
function ListSqlS2($sort,$word)
{
  ...
  $str="WHERE ETC12='ETC12:公開中' AND ETC04 like '%ETC04:研究者向け%'";
