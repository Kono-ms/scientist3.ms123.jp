...
function DispData($mode,$sort,$word,$key,$page,$lid,$token)
{
	...
	print $str;
		...
		$StrSQL="SELECT *, DAT_BLOG1.ID as BID FROM DAT_BLOG1 inner join DAT_M1 on DAT_M1.MID=DAT_BLOG1.MID inner join DAT_IINE on DAT_BLOG1.MID=DAT_IINE.MIDT and DAT_BLOG1.ENABLE='ENABLE:公開中' group by DAT_BLOG1.ID order by DAT_BLOG1.ID desc limit 0, 30;";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		...
		if($item=="") {
			...
			while ($item = mysqli_fetch_assoc($rs)) {
				...
				$str=str_replace("[O1_BID]", $item['BID'], $str);
