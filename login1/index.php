...
function DispData($id,$pass)
{
	...
	if($_GET['status']=="authorized"){
		...
		$StrSQL="SELECT * from DAT_M1 where SOCIALID='".trim($lineid)."' and ENABLE='ENABLE:公開中';";
		...
		if($item>0){
			...
			$_SESSION['MNAME'] = $item['M1_DVAL22']." ".$item['M1_DVAL23'];
//			$url = $_SESSION['REFERER'];
			$url=BASE_URL . "/m_message1/";
			header("Location: {$url}");
			exit;
		}
	}
	...
	$errmsg="";
	if($id!=""){
		//$StrSQL="SELECT ID, MID, M1_DVAL01 from DAT_M1 where EMAIL='".trim($id)."' and PASS='".trim($pass)."' and ENABLE='ENABLE:公開中';";
		$StrSQL="SELECT * from DAT_M1 where EMAIL='".trim($id)."' and PASS='".trim($pass)."' ;";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item=mysqli_num_rows($rs);
		if($item>0){
			$item = mysqli_fetch_assoc($rs);
			...
			$errmsg="<font style='color:#ff0000;'>Your e-mail address or password is invalid. Please try again.</font>";
		}
	} else {
