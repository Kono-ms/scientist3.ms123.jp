...
function DispData($id,$pass)
{
	...
	//各テンプレートファイル名
	$htmlnew = "edit.html";
	//$htmlnew = set_basic_authentication('edit.html','auth.html'); // 疑似BASIC認証

	//https://scientist3.ms123.jp/login2/?test=11111
	if($_GET['test']=="11111"){
		$StrSQL="SELECT * from DAT_M2 where EMAIL='test21@test.com'";
	...
	if($_GET['status']=="authorized"){
		...
		$StrSQL="SELECT ID, MID, EMAIL, M2_DVAL01 from DAT_M1 where SOCIALID='".trim($lineid)."' and ENABLE='ENABLE:公開中';";
		$rs=mysqli_query(ConnDB(),$StrSQL);
...
//			$url = $_SESSION['REFERER'];
	...
	$errmsg="";
	if($id!=""){
		$StrSQL="SELECT ID, MID, EMAIL, M2_DVAL01, M2_DVAL02, M2_DVAL03 from DAT_M2 where EMAIL='".trim($id)."' and PASS='".trim($pass)."' and ENABLE='ENABLE:公開中';";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item=mysqli_num_rows($rs);
		if($item>0){
			$item = mysqli_fetch_assoc($rs);
