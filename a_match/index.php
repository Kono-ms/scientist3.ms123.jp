...
function Main()
{
	...
	switch ($mode){
		case "match":
			CulcMatching($_POST['OID1'], $_POST['OID2'], 1);
			exit;
			break;
		case "all":
			$tmp="";
			...
			$tmp="";
			$StrSQL="SELECT OID FROM DAT_O2 order by OID;";
			$rs=mysqli_query(ConnDB(),$StrSQL);
			while ($item = mysqli_fetch_assoc($rs)) {
				if($tmp!=""){
					$tmp.="::";
			...
			$StrSQL="DELETE FROM DAT_MATCH;";
			if (!(mysqli_query(ConnDB(),$StrSQL))) {
				die;
			}

			for($i=0; $i<count($oid1); $i++){
				for($j=0; $j<count($oid2); $j++){
					CulcMatching($oid1[$i], $oid2[$j], 0);
				}
			}
			$mode="list";
			if ($page==""){
...
function DispData($mode,$sort,$word,$key,$page,$lid,$token)
{
	...
	print $str;
		...
		if($item=="") {
			$pagestr="";
			$strMain="<tr><td align=center colspan=11>対象データがありません。</td></tr>";
		} else {
			//================================================================================================
			//ページング処理
			//================================================================================================
		...
		$tmp="";
		$StrSQL="SELECT * FROM DAT_O1 order by OID desc;";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		while ($item = mysqli_fetch_assoc($rs)) {
			$tmp.="<option value=\"".$item['OID']."\">".$item['O1_DVAL01']."(".$item['OID'].")</option>";
		}
		$str=str_replace("[SEL-OID1]",$tmp,$str);

		$tmp="";
		$StrSQL="SELECT * FROM DAT_O2 order by OID desc;";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		while ($item = mysqli_fetch_assoc($rs)) {
			$tmp.="<option value=\"".$item['OID']."\">".$item['O2_DVAL01']."(".$item['OID'].")</option>";
		}
		$str=str_replace("[SEL-OID2]",$tmp,$str);
