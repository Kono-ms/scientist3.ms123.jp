		...
		case "save":
			...
			if ($_SESSION['token']==$token) {
				...
				if ($msg==""){
					...
					$url=BASE_URL . "/a_test/?word=".$word."&page=".$page;
					header("Location: {$url}");
					//$mode="list";
					//if ($page==""){
					//	$page=1;
		...
		if($FieldValue[34]=="見積り送付" || $FieldValue[34]=="運営手数料追加"){
			...
			while($preview_item = mysqli_fetch_assoc($preview_rs)){
				...
				if( ($preview_item["M2_PAY_TYPE"]=="Once" || $preview_item["M2_PAY_TYPE"]=="Milestone") &&
					...
					$redundancy_num<=0 ){
					//送信ボタン付きで表示
					$opt_preview_list_r.="<option value='/a_test/?mode=disp_frame1&preview_type=r&btn_version=2&key=".$preview_item["ID"]."'>";
					$opt_preview_list_r.=$SCNo_str."-Version".$preview_m2_version."</option>";
					
					//送信ボタンない状態で表示
					$opt_preview_list_cb.="<option value='/a_test/?mode=disp_frame1&preview_type=cb&btn_version=1&key=".$preview_item["ID"]."'>";
					...
					if($part=="Part0"){
						...
						$opt_preview_list_r.="<option value='/a_test/?mode=disp_frame1&preview_type=r&btn_version=2&key=".$preview_item["ID"]."'>";
						$opt_preview_list_r.=$SCNo_str."-Version".$preview_m2_version."</option>";
						$opt_preview_list_cb.="<option value='/a_test/?mode=disp_frame1&preview_type=cb&btn_version=2&key=".$preview_item["ID"]."'>";
						$opt_preview_list_cb.=$SCNo_str."-Version".$preview_m2_version."</option>";
					}else{
						//送信ボタンない状態で表示
						$opt_preview_list_r.="<option value='/a_test/?mode=disp_frame1&preview_type=r&btn_version=1&key=".$preview_item["ID"]."'>";
						$opt_preview_list_r.=$SCNo_str."-Version".$preview_m2_version."</option>";
						$opt_preview_list_cb.="<option value='/a_test/?mode=disp_frame1&preview_type=cb&btn_version=1&key=".$preview_item["ID"]."'>";
						$opt_preview_list_cb.=$SCNo_str."-Version".$preview_m2_version."</option>";
					}
		...
		if($FieldValue[34]=="決済者発注承認" || $FieldValue[34]=="発注依頼"){
			...
			while($preview_item = mysqli_fetch_assoc($preview_rs)){
				...
				if($FieldValue[34]=="決済者発注承認"){
					//送信ボタンある状態で表示
					$opt_preview_list_h.="<option value='/a_test/?mode=disp_frame2&preview_type=h&btn_version=2&key=".$preview_item["ID"]."'>";
					...
					//送信ボタンある状態で表示
					$opt_preview_list_h.="<option value='/a_test/?mode=disp_frame2&preview_type=h&btn_version=1&key=".$preview_item["ID"]."'>";
					$opt_preview_list_h.=$SCNo_str."-Version".$preview_m2_version."</option>";
				}
...
function sendPDF($str,$to){
	...
	//	Close
	//</a><br>
	//<a href="/a_test/?mode=disp_frame1&preview_type=cb&pdf_action=download&key=[KEY]">
	//	Download
	//</a><br>';
	//echo $btn_area."<br>";
...
//機能 DBにレコードを保存
//引数 a_testのレコード（発注書プレビュー用につかっている「見積り送付」のレコード）のID($key):
//戻値 $function_ret
//=========================================================================================================
...
//	$StrSQL.=" AND STATUS='発注依頼' AND DIV_ID='".$param_div_id."' order by ID desc;";
//	echo('<!--'.$StrSQL.'-->');
//	$rs=mysqli_query(ConnDB(),$StrSQL);
...
//機能 DBにレコードを保存
//引数 a_testのレコード（「納品確認」or「受注承認(一括前払い)」or「受注承認(前払い)」のレコード）のID($key)
...
//機能 DBにレコードを保存
//引数 a_testのレコード（「納品確認」のレコード）のID($key):
//戻値 $function_ret
//=========================================================================================================
