...
function Main()
{
	...
	switch ($mode){
		case "new":
			...
			//echo "mid at new:".$FieldValue[1]."<br>";
...
function SendMail()
{
	...
	$maildata1 = GetMailTemplate('CompanyInformationの変更(M1)');
	$maildata2 = GetMailTemplate('CompanyInformationの変更');
...
function DispData($mode,$sort,$word,$key,$page,$lid)
{
  ...
  $htmllist = "./contact_list.html";
	...
	for ($i=0; $i<=$FieldMax; $i=$i+1){
		...
		$str=str_replace("[L-HNAME]",convert_mid($FieldValue[1]),$str);
