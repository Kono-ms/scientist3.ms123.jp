	...
	switch ($mode){
		...
		case "save":
			...
			if($type=="サプライヤーキャンセル承認"){
				$url=BASE_URL . "/m_contact1_test/?type=サプライヤーキャンセル承認&mode=end&shodan_id=".$shodan_id;
				header("Location: {$url}");
			}else if($type=="サプライヤーキャンセル否認"){
				$url=BASE_URL . "/m_contact1_test/?type=サプライヤーキャンセル否認&mode=end&shodan_id=".$shodan_id;
				header("Location: {$url}");
			}else if($type=="キャンセル承認"){
				$url=BASE_URL . "/m_contact1_test/?type=キャンセル承認&mode=end&shodan_id=".$shodan_id;
				header("Location: {$url}");
			}
...
echo "<!--2a-->";
	  ...
	  //echo "<!--DBからデータ取得-->";
		...
		//見積り送付のデータをとってくる
		$StrSQL="SELECT ID,DIV_ID,STATUS,M2_CURRENCY FROM DAT_FILESTATUS ";
  ...
  // ２．ファイルステータスを作成
	...
	if($shodan_id != '') {
		...
		*/
		...
		//M2側からm_contact1_testにアクセスして新規作成するルートがどこかにある可能性はある。
	$StrSQL = "
		...
		foreach ($ary_div_id as $div_id) {
			...
			switch($type) {
				...
				if($item_filestatus["M2_PAY_TYPE"]=='Milestone' && $part!="" && $item_name!="" && $part!="Part1"){
					...
					<a href="javascript:window.parent.open_mcontact2(\'\'/m_contact1_test/?type=見積り送付&mode=disp_frame&key='.$key.'\'\');">
					...
					'　<a href="/m_contact1_test/?type=見積り送付&mode=new&key='.$part0_key.'&upd_mode=1' . '" target="_top">Revise Quotation</a>
					';
					...
					'　<a href="/m_contact1_test/?type=見積り送付&mode=new&key='.$key.'&upd_mode=1' . '" target="_top">Revise Quotation</a>
					';
...
//				}
				...
				if($part!="Part0"){
					...
					<a href="javascript:window.parent.open_mcontact2(\'\'/m_contact1_test/?type=追加見積り&mode=disp_frame&key='.$key.'\'\');">
					...
					'　<a href="/m_contact1_test/?type=追加見積り&mode=new&key='.$key.'&upd_mode=1' . '" target="_top">Revise Quotation</a>
					';
				}
				break;
			}
	...
	switch($type) {
		case '見積り依頼':
			...
			<a href="javascript:window.parent.open_mcontact2(\'\'/m_contact1_test/?type=見積り依頼&mode=disp_frame&key='.$key.'\'\');">
				Quote Request
			</a>
			';
		break;
		case '再見積り依頼':
			$comment = $_POST['M1_MESSAGE'] . '
			<a href="javascript:window.parent.open_mcontact2(\'\'/m_contact1_test/?type=再見積り依頼&mode=disp_frame&key='.$key.'\'\');">
...
//		break;
		case '発注依頼':
			...
			<a href="javascript:window.parent.open_mcontact2(\'\'/m_contact1_test/?type=発注依頼&mode=disp_frame&key='.$key.'\'\');">
		...
		case 'データ納品':
			...
			<a href="javascript:window.parent.open_mcontact2(\'\'/m_contact1_test/?type=データ納品&mode=disp_frame&key='.$key.'\'\');">
		...
		case '物品納品':
			$comment = $_POST['N_MESSAGE'] . '
			<a href="javascript:window.parent.open_mcontact2(\'\'/m_contact1_test/?type=物品納品&mode=disp_frame&key='.$key.'\'\');">
				Report '.$param_div_id.'
			</a>
			';
		break;
		case '請求':
			...
			<a href="javascript:window.parent.open_mcontact2(\'\'/m_contact1_test/?type=請求&mode=disp_frame&key='.$key.'\'\');">
...
//		break;
		...
		case '請求':
			$comment = $_POST['S_MESSAGE'] . '
	      <a href="javascript:window.parent.open_mcontact2(\'\'/m_contact1_test/?type=請求&mode=disp_frame&key='.$key.'\'\');">Invoice</a>
			';
  	  break;
		*/
	}
