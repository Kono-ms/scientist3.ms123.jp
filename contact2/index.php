		...
		case "new":
			...
			if($_SESSION['MATT'] == "1"){
				...
				if($item>0){
					...
					$FieldValue[2] = $item["M1_DVAL03"]; // お名前
...
function SendMail()
{
	...
	// $maildata = GetMailTemplate('お問い合わせ');
	$maildata = GetMailTemplate('Contact Us（M1）');
	$MailBody = $maildata['BODY'];
	$subject = $maildata['TITLE'];

	$MailBody=replace_mailbody($MailBody);
	$subject=replace_mailbody($subject);

	$maildataAdmin = GetMailTemplate('Contact Us(ADMIN)');
	$MailBodyAdmin = $maildataAdmin['BODY'];
	$subjectAdmin = $maildataAdmin['TITLE'];

	$MailBodyAdmin=replace_mailbody($MailBodyAdmin);
	$subjectAdmin=replace_mailbody($subjectAdmin);	


	mb_language("Japanese");
	mb_internal_encoding("UTF-8");
	mb_send_mail($FieldValue[3], $subject, $MailBody, "From:".mb_encode_mimeheader(mb_convert_encoding(SENDER_NAME,"ISO-2022-JP","AUTO"))."<".SENDER_EMAIL.">"); 

	$mailtoAdmin=SENDER_EMAIL;
	// $mailtoAdmin = "toretoresansan00@gmail.com";
	mb_send_mail($mailtoAdmin, $subjectAdmin, $MailBodyAdmin, "From:".mb_encode_mimeheader(mb_convert_encoding(SENDER_NAME,"ISO-2022-JP","AUTO"))."<".SENDER_EMAIL.">"); 
}
