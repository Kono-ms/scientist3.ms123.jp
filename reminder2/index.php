...
function DispData($id,$pass)
{
	...
	if($id!=""){
		//$StrSQL="SELECT * from DAT_M2 where EMAIL='".$id."' and ENABLE='ENABLE:公開中';";
		$StrSQL="SELECT * from DAT_M2 where EMAIL='".$id."';";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item=mysqli_num_rows($rs);
		if($item>0){
			// 時間チェックのため更新日を更新する
			$StrSQL=" UPDATE DAT_M2 SET EDITDATE='".date('Y/m/d H:i:s')."' ";
			$StrSQL.=" WHERE EMAIL = '".$id."'";
...
function SendMail($item)
{
	...
	$maildata = GetMailTemplate('パスワード再送信(M2)');
