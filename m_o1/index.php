...
//			RequestData($obj,$a,$b,$key,$mode);
		...
		case "save":
			...
			if ($_SESSION['token']==$token) {
				...
				$tmp="";
				$StrSQL="SELECT OID FROM DAT_O1 where MID='".$_SESSION['MID']."' order by OID;";
...
function SaveData($key)
{
	...
	$item=mysqli_num_rows($rs);
	if($item=="") {
		$StrSQL="SELECT OID from DAT_O1 where MID='".$_SESSION['MID']."' order by OID desc limit 0,1;";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item = mysqli_fetch_assoc($rs);
		$FieldValue[1]=$_SESSION['MID']."-".sprintf("%02d", str_replace($_SESSION['MID']."-", "", $item['OID'])+1);
