...
function Main()
{
	...
	$TITLE = '(タイトルが入ります)';
	//$StrSQL="SELECT COMMENT FROM DAT_MESSAGE where AID='".$_SESSION['MID']."-".$word."' and ETC02='".$_GET['etc02']."' and COMMENT like '[Change title]%' order by ID desc;";
...
//var_dump($StrSQL);
	...
	switch ($mode){
		...
		case "edit": 
			...
			$FieldValue[2]=str_replace("-","",str_replace($_SESSION['MID'],"",$word));
		...
		case "save":
			...
			if ($_SESSION['token']==$token) {
				...
				if ($msg==""){
					...
					//ETC03取得
					$eid = "";
					$StrSQL="SELECT MAX(ifnull(ETC03,'')) as eid FROM DAT_MESSAGE where AID='".$word."' AND ETC02 = '".$_GET['etc02']."';";
					$rs=mysqli_query(ConnDB(),$StrSQL);
					$item = mysqli_fetch_assoc($rs);
					$eid = $item["eid"];
					...
					$StrSQL.="'[Change title]<br>「".$_POST['TITLE']."」 has been entered.',";
					$StrSQL.="'".$eid."',";
					$StrSQL.="'".$_GET['etc02']."'";
					$StrSQL.=")";
					...
					// //見積
					// $StrSQL=" UPDATE DAT_ESTIMATES SET TITLE = '".$_POST['TITLE']."'";
					// $StrSQL.=" WHERE M1ID = '".$_POST['MID']."'";
					// $StrSQL.=" AND EID = '".$eid."'";
					// if (!(mysqli_query(ConnDB(),$StrSQL))) {
					// 	die;
					// }
					...
					// //請求
					// $StrSQL=" UPDATE DAT_INVOICE SET TITLE = '".$_POST['TITLE']."'";
					// $StrSQL.=" WHERE M1ID = '".$_POST['MID']."'";
					// $StrSQL.=" AND EID = '".$eid."'";
					// if (!(mysqli_query(ConnDB(),$StrSQL))) {
					// 	die;
