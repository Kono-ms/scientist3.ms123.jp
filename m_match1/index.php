...
function DispData($mode,$sort,$word,$key,$page,$lid,$token,$sel1,$sel2)
{
	...
	if($item=="") {
		$reccount=0;
		$pagestr="";
		$strMain="<div class='result__item'><p class='result__ttl'>No search results were found.<p></div><div class='result__item'><p class='result__txt'>Supplierの募集情報の登録はお済みでしょうか？検索結果の表示にはSupplierの募集情報の登録が必要になります。まだ未登録の場合は先にこちらから登録をお願いします。</p><div class='btn result__btn'><a href='/m_o1/?mode=new&sort=&word=[L-MID]&page=1'>Supplierの募集情報を作成する</a></div></div>";
	} else {
		//================================================================================================
		//ページング処理
		//================================================================================================
