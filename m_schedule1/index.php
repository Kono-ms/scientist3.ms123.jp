...
function Main()
{
	...
	if($mode=="disp"){
		...
		if($key==""){
			...
			$key=$item['ID'];
			if($key==""){
				print "自分のプロフィールを見るには、Supplierの募集情報を登録してください。";
				exit;
			}
		}
...
function SendMail($midt)
{
	...
	//mb_send_mail($item['EMAIL'], "【Scientist3】あなたのSupplierの募集情報情報が「いいね」されました！", $MailBody, "From:".mb_encode_mimeheader(mb_convert_encoding("Scientist3事務局","ISO-2022-JP","AUTO"))."<info@msc-dev.com>"); 
...
function DispData($mode,$sort,$word,$key,$page,$lid,$token,$sel1,$sel2,$word2,$mid1)
{
	...
	print $str;
		...
		$filename="../common/template/list_schedule1.html";
		$fp=$DOCUMENT_ROOT.$filename;
		...
		*/
		...
		//	WHERE
		//		DAT_FILESTATUS.MID1 = '".$_SESSION['MID']."' 
		//		 and (DAT_FILESTATUS.STATUS='見積り送付' or 
		//		 DAT_FILESTATUS.STATUS='問い合わせ' 
		//		 or DAT_FILESTATUS.STATUS='見積り依頼')
		...
		////同じSHODAN_IDで、「見積り依頼」と「見積り送付」の行があった場合、
		////「見積り依頼」の行はとってこず、「見積り送付」の行の方をとってくる。
		//$StrSQL = "
		//	SELECT
		//		DAT_FILESTATUS.SHODAN_ID,
		//		DAT_FILESTATUS.STATUS as STATUS,
		//		DAT_FILESTATUS.CATEGORY as CATEGORY,
		//		DAT_FILESTATUS.ID as FILESTATUS_ID,
		//		DAT_FILESTATUS.M2_ID as M2_ID,
		//		DAT_FILESTATUS.M2_VERSION as M2_VERSION,
		//		DAT_FILESTATUS.M2_PAY_TYPE as M2_PAY_TYPE,
		...
		//「見積り依頼」の行はとってこず、「見積り送付」の行の方をとってくる。
		//新設ステータス「運営手数料追加」対応と、
		//見積り送付以降の「Estimate」表記の1行はDIV_IDが同じものを指定する。
		$StrSQL = "
			SELECT
				DAT_FILESTATUS.SHODAN_ID,
				DAT_FILESTATUS.STATUS as STATUS,
		...
		if($item=="") {
			...
			$strMain="<div class='result__item'><p class='result__ttl'>No search results were found.<p></div>";
			$start = 0;
			$end = 0;
		} else {
			//================================================================================================
			...
			while ($item = mysqli_fetch_assoc($rs)) {
				...
				$str=str_replace("[M2_ETC20]",$item['M2_ETC20'],$str);


				if(strpos($item['STATUS'], '再見積り依頼') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('再見積り依頼'),$str);
				}
				else if(strpos($item['STATUS'], '見積り依頼') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('見積り依頼'),$str);
				}
				else if(strpos($item['STATUS'], '問い合わせ') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('問い合わせ'),$str);
				}
				...
				if(strpos($shodan_item['STATUS'], '完了') !== false) {
					...
					$str=str_replace("[CATEGORY]",showStatus('サプライヤーキャンセル承認'),$str);
				}
				else if(strpos($shodan_item['STATUS'], 'サプライヤーキャンセル承認（追加見積り）') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('サプライヤーキャンセル承認（追加見積り）'),$str);
				}
				else if(strpos($shodan_item['STATUS'], 'サプライヤーキャンセル否認') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('サプライヤーキャンセル否認'),$str);
				}
				else if(strpos($shodan_item['STATUS'], 'キャンセル承認') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('キャンセル承認'),$str);
				}
				else if(strpos($shodan_item['STATUS'], 'キャンセル承認（請求あり）') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('キャンセル承認（請求あり）'),$str);
				}
				else if(strpos($shodan_item['STATUS'], 'キャンセル') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('キャンセル'),$str);
				}
				else if(strpos($shodan_item['STATUS'], '辞退') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('辞退'),$str);

				}else if(strpos($shodan_item['STATUS'], '請求書送付(一括前払い)') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('実施中'),$str);
				}
				else if(strpos($shodan_item['STATUS'], '請求') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('請求'),$str);
				}
				else if(strpos($shodan_item['STATUS'], 'サプライヤーが納品(一括前払い)') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('納品'),$str);
				}
				...
				if($item["STATUS"]=="見積り送付" || $item["STATUS"]=="運営手数料追加"){
					...
					//$status_m .= '<div><a href="/m_contact1/?type=見積り送付&mode=disp&key='.$item["FILESTATUS_ID"].'">〇（Version'.$item["M2_VERSION"	].'）</a></div>';
		...
		//以下は仕様変更前のバージョンv1とする
		  ...
		  SELECT
			  ...
			  group_concat(DAT_FILESTATUS.M2_ID) as M2_ID,
			  group_concat(DAT_FILESTATUS.M2_VERSION) as M2_VERSION,
				DAT_M2.M2_DVAL01
			FROM
				DAT_FILESTATUS
				join DAT_M2
		...
		if($item=="") {
			...
			while ($item = mysqli_fetch_assoc($rs)) {
				...
				else if(strpos($item['STATUS'], '請求') !== false) {
					$str=str_replace("[CATEGORY]",showStatus('請求'),$str);
				}
				...
				foreach($s1 as $key => $s2) {
					...
					if($s2 == 'データ納品') {
						$status_n .= '<div><a href="/m_contact1/?type=データ納品&mode=disp&key='.$f1[$key].'">〇（data）</a></div>';
					}
					if($s2 == '物品納品') {
						$status_n .= '<div><a href="/m_contact1/?type=物品納品&mode=disp&key='.$f1[$key].'">〇（Goods）</a></div>';
					}
					if($s2 == '見積り送付') {
...
function MakeUrl($sort,$word, $sel1, $sel2,$page)
{
	...
	return "/m_schedule1/?mode=list&sort=".urlencode($sort)."&word=".urlencode($word)."&page=".urlencode($page)."&sel1=".urlencode($sel1)."&sel2=".urlencode($sel2);
