	...
	switch ($mode){
		case "new":
			...
			if($item>0){
				...
				$FieldValue[8] = $item["M2_DVAL07"]; // 会社住所
				$FieldValue[9] = $item["M2_DTXT02"]; // 会社ホームページURL

				$FieldValue[2] = $item["M2_DVAL16"]." ".$item["M2_DVAL17"]; //氏名
				$FieldValue[3] = $item["EMAIL"]; //mailto、送信者
				$FieldValue[10] =convert_mid($item["MID"]); // 研究者ID
...
function DispData($mode,$sort,$word,$key,$page,$lid)
{
  ...
  $htmllist = "./contact_list.html";
	...
	if ($mode!="list"){
		...
		//契約フラグ:M2_DSEL02
		//$StrSQL="SELECT ID, MID, M2_DSEL02 from DAT_M2 where ID = '".$_SESSION['M-ID']."';";
		$StrSQL="SELECT ID, MID, M2_DSEL02 from DAT_M2 where MID = '".$_SESSION['MID']."';";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item=mysqli_fetch_assoc($rs);

		if($item["M2_DSEL02"]=="M2_DSEL02:確認中"){
			$str=DispParamNone($str,"NOT_KAKUNIN");
			$str=DispParam($str,"KAKUNIN");
			$str=DispParamNone($str,"SUMI");
		}else if($item["M2_DSEL02"]=="M2_DSEL02:済"){
			$str=DispParam($str,"SUMI");
			$str=DispParamNone($str,"NOT_KAKUNIN");
			$str=DispParamNone($str,"KAKUNIN");
		}else{
			$str=DispParam($str,"NOT_KAKUNIN");
			$str=DispParamNone($str,"KAKUNIN");
			$str=DispParamNone($str,"SUMI");
		}
