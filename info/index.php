...
function DispData($mode,$sort,$word,$key,$page,$lid)
{
		...
		else
	{
		...
		$StrSQL="";
		$StrSQL="SELECT * FROM ".$TableName." ".ListSqlS($sort,$word)." LIMIT 0, 100;";
		$rs=mysqli_query(ConnDB(),$StrSQL);
...
//<li style=\"padding:10px 0px;border-bottom:0px;\"><div class=\"info_lst\"><p class=\"info_lst_fl\"><a href=\"/info/".$item['URL']."/\"><img src=\"".$item['CPIC']."\" alt=\"".$item['CNAME']."\" width=\"160\" /></a></p><p class=\"info_lst_fr\"><a href=\"/info/".$item['URL']."/\">".$item['CCOMMENT']."</a></p></div></li>";
	...
	$tmp="";
	$StrSQL="SELECT * FROM DAT_INFO where CCATE like '".mb_substr($cate,0,10,"UTF-8")."%' and ETC12='ETC12:公開中' order by DATE desc, ID desc LIMIT 0, 8;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
	...
	$cnt=1;
	$tmp="";
	$StrSQL="SELECT * FROM DAT_INFO where CCATE<>'CCATE:特集' and ETC12='ETC12:公開中' order by ETC12, DATE desc, ID desc LIMIT 0, 10;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
