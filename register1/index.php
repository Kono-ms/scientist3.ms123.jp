...
// echo "<!--mode:".$mode."-->"; 
	...
	print $str;
			...
			$pass=$FieldValue[3];
			// if(strlen($pass)<8){
			// 	$emsg.="<span>8文字以上で入力してください。</span><br>";
			// }
			// $result = preg_match('/^[a-zA-Z0-9`˜!@#$%^&*()_+-={}[]\|:;"\'<>,.?\\]*$/', $pass);
			// if($result){

			// } else {
			// 	$emsg.="<span>半角英数字記号で入力してください。</span><br>";
			// }	
			
			// if($_POST["PASS2"]!=$pass){
			// 	$emsg.="<span>パスワードが一致しません</span><br>";
			// }
			

			// if($emsg==""){
				$FieldValue[22]=date("Y/m/d H:i:s");//利用規約に同意した日時（仮登録） M1_DTXT08
			// } else {
			// 	$mode="edit";
			// }
	...
	// 		}
			...
			//フリーメールチェック
			// $emails = array($FieldValue[2],$FieldValue[13]);
			$emails = array($FieldValue[2]);
...
// $freemail=0; //test
	...
	// 		}
			...
			// 次へではなくここで登録して終わりに仕様変更
			$StrSQL="SELECT MID from DAT_M1 where MID !='".M1_SYSTEM_MID."' order by MID desc limit 0,1;";
			$rs=mysqli_query(ConnDB(),$StrSQL);
			...
			//$_SESSION['MNAME'] = $FieldValue[129]." ".$FieldValue[130];

			$FieldValue[35]="M1_DRDO01:仮登録中";

			////M1_DVAL04 Country
			//if(strpos($FieldValue[8], $FieldName[8])===false){
			//	$FieldValue[8]=$FieldName[8].":".$FieldValue[8];
			//}
...
//			}
	...
	DispData($mode,$sort,$word,$key,$page,$lid,$token,$emsg);
...
function SendMail()
{
	...
	$maildata = GetMailTemplate('会員登録完了(M1)');
	$MailBody = $maildata['BODY'];
	$subject = $maildata['TITLE'];

	$MailBody=str_replace("[EMAIL]",$FieldValue[2],$MailBody);
	$MailBody=str_replace("[PASSWORD]",$FieldValue[3],$MailBody);

	$MailBody=str_replace("[M1_DVAL01]",$FieldValue[5],$MailBody);
	$MailBody=str_replace("[M1_DVAL22]",$FieldValue[129],$MailBody);
	$MailBody=str_replace("[M1_DVAL23]",$FieldValue[130],$MailBody);
...
//戻値 なし
//=========================================================================================================
function DispData($mode,$sort,$word,$key,$page,$lid,$token,$emsg)
{
	...
	// if($_GET['status']!="authorized"){
		...
		$htmlerr = "edit.html";
		$htmledit = "edit.html";
		$htmledit2 = "edit2.html";
		$htmledit3 = "edit3.html";
		$htmledit4 = "edit4.html";
		$htmlconf = "conf.html";
		$htmlend = "end.html";
		$htmldisp = "disp.html";
		$htmllist = "list.html";
	...
	for ($i=0; $i<=$FieldMax; $i=$i+1){
		...
		if ($FieldAtt[$i]=="1"){
			...
			for ($j=0; $j<count($tmp); $j=$j+1) {
				if($tmp[$j] == '----------') {
					$strtmp=$strtmp."<option value=''>".$tmp[$j]."</option>";
				}
				else if($FieldName[$i] == 'M1_DSEL03') { // datalist
					$strtmp=$strtmp."<option value='".$tmp[$j]."'>".$tmp[$j]."</option>";
				}
				//else if($FieldName[$i] == 'M1_DVAL04') { // datalist
				//	//データ保存時はvalueの先頭に「M1_DVAL04:」を付ける
				//	$strtmp=$strtmp."<option value='".$tmp[$j]."'>".$tmp[$j]."</option>";
				//}
				else {
			...
			if (($filename==$htmlerr || $mode=="new" || $mode=="edit" || $mode=="edit2" || $mode=="edit3" || $mode=="edit4") && $FieldValue[$i]!="") {
...
function RequestData($obj,$a,$b,$key,$mode)
{
	...
	for ($i=0; $i<=$FieldMax; $i=$i+1) {
		...
		if ($FieldAtt[$i]==4 && ($mode=="saveconf" || $mode=="edit" || $mode=="edit2" || $mode=="edit3" || $mode=="edit4")) {
			...
			if ($filename!="s.gif" && ($extention!="" && !!isset($extention))) {
				$uploadpath="../a_m1/data/". $filename;
// echo "<!--".$uploadpath."-->";
				if(move_uploaded_file($_FILES["EP_".$FieldName[$i]]["tmp_name"], $uploadpath)){
// echo "<!--move_uploaded_file成功-->";
				} else {
// echo "<!--move_uploaded_file失敗-->";
				}
...
function SaveData($key)
{
	...
	// ①サプライヤー会員登録で(register1)で会員登録
	// ②M299999のアカウント（既に作成済み）との商談データ(DAT_SHODAN)も同時に生成。ステータスは「問い合わせ」
	// ③/m_message1/にM299999とのメッセージスレッドが表示される
