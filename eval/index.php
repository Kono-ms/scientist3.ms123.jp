...
function Main()
{
	...
	// 2021.01.18 yamamoto パラメータチェック
	if($mode == 'new') {
		if(!isset($_GET['contact_id']) || !isset($_GET['mid1']) || !isset($_GET['mid2'])) {
		$mode = "eval_error";
		}
		else {
			$StrSQL="SELECT ID FROM DAT_MCONTACT where ID='".$_GET['contact_id']."' and MID='".$_GET['mid1']."' and  MIDT='".$_GET['mid2']."' and STATUS='STATUS:納品完了';";
...
function DispData($mode,$sort,$word,$key,$page,$lid,$token)
{
	...
	// 2021.01.18 yamamoto パラメータエラー
	$htmlevalerr = "error.html";
	...
	if ($mode!="list"){
		...
		// 2021.01.18 yamamoto hidden埋め
		$str=str_replace("[MID1]",$_GET['mid1'],$str);
		$str=str_replace("[MID2]",$_GET['mid2'],$str);
		$str=str_replace("[DATE1]",date('Y-m-d H:i:s'),$str);
