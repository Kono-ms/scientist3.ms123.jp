...
function Main()
{
	...
	if ($mode=="cancel"){
		...
		if($iid!=""){
			$StrSQL="UPDATE DAT_請求書 SET ";
			$StrSQL=$StrSQL." CDATE ='".date("Y/m/d H:i:s")."',";
			$StrSQL=$StrSQL." STATUS ='STATUS:キャンセル'";
			$StrSQL=$StrSQL." WHERE IID ='".$iid."'";
	...
	if ($mode=="delete"){
		//物理削除
		$StrSQL="DELETE FROM DAT_ESTIMATES ";
		$StrSQL=$StrSQL." WHERE EID ='".$eid."'";
	...
	if ($mode=="nyukin"){
		//入金日時をセット、ステータスを入金済に変更
		$StrSQL="UPDATE DAT_ESTIMATES SET ";
		$StrSQL=$StrSQL." STATUS ='STATUS:入金済'";
		$StrSQL=$StrSQL." WHERE EID ='".$eid."'";
		...
		$StrSQL="UPDATE DAT_請求書 SET ";
		$StrSQL=$StrSQL." PDATE ='".date("Y/m/d H:i:s")."',";
		$StrSQL=$StrSQL." STATUS ='STATUS:入金済'";
		$StrSQL=$StrSQL." WHERE IID ='".$iid."'";
...
function DispData($mode,$sort,$word,$key,$page,$lid,$token,$mid1,$mid2,$word2,$word3)
{
	...
	// $filename="../common/template/chat1_list_part.html";
	...
	$hid="";
	// SQLインジェクション対策
	$StrSQL=" SELECT MSG.*,ifnull(MSG_NOREAD.NOREAD,'zzz') as NOREAD,DAT_M2.ID as M2KEY,DAT_M2.M2_DVAL01 as M2NAME, ";
	...
	$StrSQL.=" (SELECT MSG1.ETC04 as star,MSG1.ETC02 as msgid,DAT_MESSAGE.AID,DAT_MESSAGE.NEWDATE as ldate, DAT_MESSAGE.COMMENT as MSG from DAT_MESSAGE INNER JOIN ";
	$StrSQL.="    (SELECT ETC02,max(ifnull(ETC04, '')) as ETC04,AID,max(NEWDATE) as ldate from DAT_MESSAGE where AID = '".$_GET['word']."' and ETC02 is not null and ETC02 != '' group by AID,ETC02 ) MSG1 ";
	...
	$StrSQL.=" replace(replace(DAT_MESSAGE.COMMENT,'[Change title]<br>タイトル「',''),'」が入力されました。','') as TITLE ";
	...
	$StrSQL.=" LEFT JOIN DAT_M2 ON SUBSTR(MSG.AID,9) = DAT_M2.MID ";
	if($word2!=""){
		$StrSQL.=" where DAT_M2.M2_DVAL01 like '%".$word2."%' ";
	}
	...
	$StrSQL.=" CASE WHEN ifnull(MSG_NOREAD.NOREAD,'zzz') ='' THEN 1 ELSE 0 END desc,CASE WHEN ifnull(star,'') ='ETC04:対象' THEN 1 ELSE 0 END desc, MAXDATE desc;";
...
// var_dump($StrSQL);
	...
	if($item=="") {
		...
		while ($item = mysqli_fetch_assoc($rs)) {
			...
			//ステータス
			$comment = $item['DETAIL_MSG'];
			// if(strpos($comment,'承認') === false && strpos($comment,'否認') !== false){
			// 	$status="見積依頼否認";
			// }
...
//  var_dump($StrSQL);
			...
			while ($item2 = mysqli_fetch_assoc($rs2)) {
				...
				if($cnt==0){
					if($item2['STATUS']=="STATUS:見積依頼中"){
						$status.="<a target=\"_blank\" href=\"/m_estimates/?mode=edit&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&eid=".$item2['EID']."&key=".$item2['ID']."&etc02=".$item['msgid']."\">見積作成</a>"; //見積作成
						$mirai_cnt = $mirai_cnt + 1;
					} else {
						$status.="<a target=\"_blank\" href=\"/m_estimates/?mode=new&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&title=".$title."&etc02=".$item['msgid']."\">見積作成</a>"; //見積作成
						$mirai_cnt = $mirai_cnt + 1;
					}
				}
				...
				if($item2['STATUS']=="STATUS:見積依頼中"){
					...
					$status.="●".$item2["EID"]."　"; //見積番号
					$status.="<a target=\"_blank\" href=\"/m_estimates/?mode=hacchudisp&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&eid=".$item2['EID']."&key=".$item2['ID']."\">発注書</a>"; //発注書
					$status.="　<a href=\"/m_estimates/?mode=nouhin&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&eid=".$item2['EID']."&key=".$item2['ID']."\">納品</a>"; //納品
					
					$hacchu_cnt = $hacchu_cnt + 1;
				} else if($item2['STATUS']=="STATUS:納品済"){
					$status.="●".$item2["EID"]."　"; //見積番号
					$status.="<a href=\"/m_請求書/?mode=new&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&eid=".$item2['EID']."&key=".$item2['ID']."&etc02=".$item['msgid']."\">請求作成</a>"; //請求作成
					$nohin_cnt = $nohin_cnt + 1;
				} else if($item2['STATUS']=="STATUS:請求提示中"){
					$status.="●".$item2["EID"]."　"; //見積番号
					$status.="請求提示中"; //請求提示中
					$status.="　<a onclick=\"var ok=confirm('入金済みしてよろしいですか？');if (ok) this.form.submit(); return false;\" href=\"./?mode=nyukin&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&eid=".$item2['EID']."&iid=".$item2['IID']."&key=".$item2['ID']."\">入金済み</a>"; //入金済み
					$status.="　<a onclick=\"var ok=confirm('キャンセルしてもよろしいですか？');if (ok) this.form.submit(); return false;\" href=\"./?mode=cancel&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&eid=".$item2['EID']."&iid=".$item2['IID']."&key=".$item2['ID']."\">キャンセル</a>"; //キャンセル
			...
			if($word3=="見積依頼中"){
				if (strpos($status, "見積作成") === false)
				{
					continue;
				}
			}
			if($word3=="見積提示中"){
				if (strpos($status, "見積提示中") === false)
				{
					continue;
				}
			}
			if($word3=="発注済"){
				if (strpos($status, "納品") === false)
				{
					continue;
				}
			}
			if($word3=="納品済"){
				if (strpos($status, "請求作成") === false)
				{
					continue;
				}
			}
			if($word3=="請求提示中"){
				if (strpos($status, "請求提示中") === false)
				{
					continue;
				}
			}
		...
		// var_dump("postion:".$postion);
		mysqli_data_seek($rs, $postion);
		...
		for ($i=$s; $i<=$e; $i=$i+1) {
			if ($i==intval($page)) {
				...
				$str.="<li class=\"paginate_button\"><a href=\"".$aspname."?mode=list&lid=".$lid."&sort=".$sort."&word=".$word."&page=".$i."&word2=".$word2."&word3=".$word3."\" aria-controls=\"table_summary\" data-dt-idx=\"\" tabindex=\"0\">".$i."</a></li>";
			} 
		}
		if (intval($page)<$pagecount) {
			$str.="<li class=\"paginate_button next\" id=\"table_summary_next\"><a href=\"".$aspname."?mode=list&lid=".$lid."&sort=".$sort."&word=".$word."&page=".($page+1)."&word2=".$word2."&word3=".$word3."\" aria-controls=\"table_summary\" data-dt-idx=\"\" tabindex=\"0\">次の".$PageSize."件</a></li>";
		} 
		...
		while ($item = mysqli_fetch_assoc($rs)) {
            ...
            if ($item['star'] =="ETC04:対象") {
				$str=str_replace("[STAR]","<a href=\"./?mode=notstar&sort=".$sort."&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&eid=".$item['EID']."&msgid=".$item['msgid']."\">★</a>",$str);
			} else {
				$str=str_replace("[STAR]","<a href=\"./?mode=star&sort=".$sort."&word=".$word."&mid1=".$mid1."&mid2=".$mid2."&eid=".$item['EID']."&msgid=".$item['msgid']."\">☆</a>",$str);
			}
			
			
	

			$str=str_replace("[M2KEY]",$item['M2KEY'],$str);
			$str=str_replace("[M2NAME]",$item['M2NAME'],$str);
...
//  var_dump($StrSQL);
	...
	$StrSQL="SELECT * FROM DAT_M2 where MID='".$_GET['mid2']."';";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item = mysqli_fetch_assoc($rs);
	$str=DispM2($item, $str);
