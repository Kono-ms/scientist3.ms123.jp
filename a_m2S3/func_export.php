...
$StrSQL="SELECT * FROM ".$TableName." WHERE 1=1 ";
	...
	if($item<>"") {
		...
		header("Content-Disposition: attachment; filename=m2".date('YmdHis').".csv");
		...
		while ($item = mysqli_fetch_assoc($rs)) {
			...
			for ($j=0; $j<=count($export_array); $j++) {
				...
				if($label!=""){
					...
					if($val=="MID_NEW"){
						$data=convert_MID($item["MID"]);
					} else if($val=="決裁者承認"){
						$kessisyaid=$item["M2_DVAL15"];

						$StrSQL="SELECT M2_DVAL01 FROM DAT_M3 WHERE MID='".$kessisyaid."'";
						$mid3_rs=mysqli_query(ConnDB(),$StrSQL);
						$mid3_item=mysqli_fetch_assoc($mid3_rs);

						$data=$mid3_item["M2_DVAL01"]; //ニックネーム
