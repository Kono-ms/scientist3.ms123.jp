...
define("LABLE_LIST", 
"会員ID::研究者ID::メールアドレス::パスワード::登録日時（利用規約同意日時）::更新日時::PF手数料（％）::契約フラグ::決裁者ID::決裁者承認::【表示項目】ニックネーム::氏名（姓）::氏名（名）::氏名（FirstName）::氏名（LastName）::組織名::組織名（英語表記）::部署名::部署名（英語表記）::役職::郵便番号::住所::住所（英語表記）::TEL::FAX::ホームページURL::興味のある研究分野（キーワード）::アカウント情報（社内メモ）"
);
define("VALUE_LIST", 
"MID::MID_NEW::EMAIL::PASS::NEWDATE::EDITDATE::M2_ETC02::M2_DSEL02::M2_DVAL15::決裁者承認::::M2_DVAL01::M2_DVAL16::M2_DVAL17::M2_DVAL18::M2_DVAL19::M2_DVAL03::M2_ETC20::M2_DVAL04::M2_DVAL21::M2_ETC19::M2_DVAL06::M2_DVAL07::M2_DVAL20::M2_DVAL08::M2_DVAL09::M2_DTXT02::M2_DTXT03::M2_ETC18"
);
...
function Main()
{
	...
	//https://scientist3.ms123.jp/a_m2S3/?mode=test1
	if ($mode=="test1"){
	...
	if ($mode==""){
		$mode="list";
		$_SESSION['a_m2_expo_ids']="";//CSV出力の選択ID（カンマ区切り）
	}
	if($mode=="expo_chk"){
		$tmps=explode(",",$_SESSION['a_m2_expo_ids']);
		if($word=="on"){
		...
		$_SESSION['a_m2_expo_ids']=implode(",",$tmps);
		exit;
	}
// echo "<!--a_m2_expo_ids:".$_SESSION['a_m2_expo_ids']."-->";
	...
	switch ($mode){
		case "enable":
			...
			if(is_array($chk)){
				for($i=0; $i<count($chk); $i++){
					$StrSQL=" UPDATE DAT_M2 SET ENABLE = '".$enable."'";
		...
		case "save":
			...
			if ($_SESSION['token']==$token) {
				LoadData($key);
				$keyaku_pre=str_replace($FieldName[26].":", "",$FieldValue[26]);
				echo "<!--pre:".$keyaku_pre."-->";
				...
				//契約フラグが前回と別のものに更新されたら、「未締結」もしくは「済」に変更されたとき、メールを飛ばす。
				if($keyaku_pre!=$keyaku_now){
					if($keyaku_now=="未締結"){
						echo "<!--メール送信：未締結-->";

					}else if($keyaku_now=="済"){
						echo "<!--メール送信：済-->";
						SendMail($key);
					}
				}

				//契約フラグが前回と別のものに更新されたら、「確認中」から「未締結」への変更の場合に、メールを飛ばす。
				if($keyaku_pre!=$keyaku_now){
					if($keyaku_pre=="確認中" && $keyaku_now=="未締結"){
						echo "<!--メール送信：確認中→未締結-->";
						SendMail2($key);
					}
				}
...
function SendMail($key)
{
	...
	$maildata = GetMailTemplate('契約締結完了のお知らせ');
...
function SendMail2($key)
{
	...
	$maildata = GetMailTemplate('契約未締結');
	$MailBody = $maildata['BODY'];
	$subject = $maildata['TITLE'];
	...
	print $str;
		...
		if($item=="") {
			...
			while ($item = mysqli_fetch_assoc($rs)) {
				...
				$str=str_replace("[MID_NEW]",convert_MID($item["MID"]),$str);
