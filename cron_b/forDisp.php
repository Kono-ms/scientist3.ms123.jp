...
//DAT_MESSAGEで研究者送信の未読メッセージで素チャットを探す。
//※ETC03に値が入ってない場合は素チャット
//※ETC03が空なので、サプライヤーのMIDはAIDから抽出するしかない。
$StrSQL="SELECT * from DAT_MESSAGE WHERE (NOREAD <=> NULL OR NOREAD='') ";
$StrSQL.="AND RID LIKE 'M2%' ";
...
while($item=mysqli_fetch_assoc($rs)){
	...
	$mid1=explode("-", $item["AID"])[0];

	//echo "aid: ".$item["AID"].", mid2: $mid1<br>";
	$StrSQL="SELECT * from DAT_M1 WHERE MID='".$mid1."' limit 1";
	$rs4=mysqli_query(ConnDB(),$StrSQL);
	$item4=mysqli_fetch_assoc($rs4);
	$email1=$item4["EMAIL"];
	if($email1=="" || is_null($email1)){
		continue;
	}

	echo "未読： ".$item["ID"].": ".$item["NEWDATE"].": shodan_id-".$item["ETC02"].": MessageFrom-".$item["RID"].": MessageTo-".$mid1.": EMAIL-".$email1."<br>\n";
	
	SendMail($item4,$item["RID"]);
